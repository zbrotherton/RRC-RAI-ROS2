...
PenVisualizer::PenVisualizer() : Node("pen_visualizer"), marker_count_(0) {
  ...
  for(int i = 0; i < 3; i++){
    ...
    state_sub_array_.at(i) = this->create_subscription<std_msgs::msg::Bool>(
      ...
      1,
      std::bind(&PenVisualizer::state_callback, this, id, std::placeholders::_1));
  }

  marker_pub_ = this->create_publisher<visualization_msgs::msg::Marker>("pen_marker", 1);
}

void PenVisualizer::state_callback(int id, const std_msgs::msg::Bool::SharedPtr msg) {
  if(msg->data){
    std::optional<visualization_msgs::msg::Marker> marker = pen_array_.at(id - 1).generate_marker(marker_count_);
    if(marker.has_value()){
